...
- CI commit-backs (e.g. the `pages.yml` screenshot commit) must carry a
  `[skip ci]` marker in the commit message so the push does not re-trigger
  workflows — GitHub honours `[skip ci]` and `[no ci]` identically (step 28).
